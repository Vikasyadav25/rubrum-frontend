...
function* fetchInwardListWithOldAPI(action) {
  const body = {
    pageNo: action.page,
  ...
  try {
    // const fetchInwardList = yield fetch(`${baseUrl}api/inwardEntry/partywise/${page}/${pageSize}?searchText=${searchValue}&partyId=${partyId}`, {
    const fetchInwardList = yield fetch(
      `${baseUrl}api/inwardEntry/inwardlist`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json", ...getHeaders() },
        body: JSON.stringify(body),
...
function* fetchInwardInstructionWIPDetails(action) {
  try {
    ...
    if (fetchInwardList.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(getInstructionWipListError("error"));
  } catch (error) {
...
function* checkCoilDuplicate(action) {
  try {
    const checkCoilDuplicate = yield fetch(
      `${baseUrl}api/inwardEntry/isCoilPresent?coilNumber=${action.coilNumber}`,
      {
        method: "GET",
        headers: getHeaders(),
      }
    ...
    if (checkCoilDuplicate.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(checkDuplicateCoilError("error"));
  } catch (error) {
...
function* checkCustomerBatchNumber(action) {
  try {
    const checkCustomerBatchNumberResponse = yield fetch(
      `${baseUrl}api/inwardEntry/isCustomerBatchIdPresent?customerBatchId=${action.customerBatchId}`,
      {
        method: "GET",
        headers: getHeaders(),
      }
...
function* submitInward(action) {
  try {
    ...
    data.append("ys", action.inward.ys);
    data.append("uts", action.inward.uts);
    data.append("el", action.inward.el);
...
function* updateInward(action) {
  try {
    const partyId =
      action.inward.partyName !== undefined
        ? action.inward.partyName
        : action.inward.party.nPartyId;
    const materialId =
      action.inward.description !== undefined
        ? action.inward.description
        : action.inward.material.matId;

    let insObj = {
      inwardId: action.inward.inwardEntryId.toString(),
      partyId: partyId.toString(),
      customerBatchId:
        action.inward.customerBatchNo || action.inward.customerBatchId || "",
      ...
      vehicleNumber:
        action.inward.vehicleNumber !== undefined
          ? action.inward.vehicleNumber
          : action.inward.vLorryNo,
      invoiceDate:
        moment(action.inward.invoiceDate).format("YYYY-MM-DD HH:mm:ss") !==
        undefined
          ? moment(action.inward.invoiceDate).format("YYYY-MM-DD HH:mm:ss")
          : null,
      invoiceNumber:
        action.inward.invoiceNumber !== undefined
          ? action.inward.invoiceNumber
          : action.inward.vInvoiceNo,
      valueOfGoods:
        action.inward.valueOfGoods !== undefined
          ? action.inward.valueOfGoods
          : 0,
      purposeType: action.inward.purposeType,
      testCertificateNumber:
        action.inward.testCertificateNo ||
        action.inward.testCertificateNumber ||
        "",
      materialId: materialId.toString(),
      width:
        action.inward.width !== undefined
          ? action.inward.width
          : action.inward.fWidth,
      thickness:
        action.inward.thickness !== undefined
          ? action.inward.thickness
          : action.inward.fThickness,
      length:
        action.inward.length !== undefined
          ? action.inward.length
          : action.inward.fLength,
      statusId:
        action.inward.status.statusId !== undefined
          ? action.inward.status.statusId
          : "1",
      heatnumber: "",
      plantname: "",
      process: "",
      presentWeight:
        action.inward.weight !== undefined
          ? action.inward.weight
          : action.inward.fQuantity,
      cast: "",
      materialGradeId:
        action.inward.grade !== undefined
          ? action.inward.grade
          : action.inward.materialGrade.gradeId.toString(),
      createdBy: "1",
      updatedBy: "2",
    };
    ...
    if (newInwardEntry.status == 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(updateInwardError("error"));
  } catch (error) {
...
function* fetchInwardListByParty(action) {
  try {
    const fetchPartyInwardList = yield fetch(
      `${baseUrl}api/inwardEntry/getByPartyId/${action.partyId}`,
      {
        method: "GET",
        headers: getHeaders(),
      }
    ...
    if (fetchPartyInwardList.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(getCoilsByPartyIdError("error"));
  } catch (error) {
...
function* fetchPartyListById(action) {
  try {
    const fetchPartyInwardList = yield fetch(
      `${baseUrl}api/inwardEntry/getById/${action.inwardEntryId}`,
      {
        method: "GET",
        headers: getHeaders(),
      }
    ...
    if (fetchPartyInwardList.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(fetchPartyInwardListByIdError("error"));
  } catch (error) {
...
function* fetchInwardPlanDetails(action) {
  try {
    const fetchInwardPlan = yield fetch(
      `${baseUrl}api/inwardEntry/getByCoilId/${action.coilNumber}`,
      {
        method: "GET",
        headers: getHeaders(),
      }
    ...
    if (fetchInwardPlan.status === 200) {
      ...
      fetchInwardPlanResponse.instruction.map((instruction) => {
        ...
        } else formattedResponse.push([instruction]);
      });
      ...
      yield put(userSignOutSuccess());
    } else yield put(getCoilPlanDetailsError("error"));
  } catch (error) {
...
function* requestSaveCuttingInstruction(action) {
  try {
    ...
    if (fetchPartyInwardList.status === 201) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(saveCuttingInstructionError("error"));
  } catch (error) {
...
function* instructionGroupsave(action) {
  try {
    ...
    if (groupSaveList.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(instructionGroupsaveError("error"));
  } catch (error) {
...
function* requestSaveSlittingInstruction(action) {
  try {
    ...
    if (fetchPartyInwardList.status === 201) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(saveSlittingInstructionError("error"));
  } catch (error) {
...
function* requestGradesByMaterialId(action) {
  try {
    const fetchGradesByMaterialIdList = yield fetch(
      `${baseUrl}api/materialGrade/getByMaterialId/${action.materialId}`,
      {
        method: "GET",
        headers: getHeaders(),
      }
    ...
    if (fetchGradesByMaterialIdList.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(getGradeByMaterialIdError("error"));
  } catch (error) {
...
function* postDeliveryConfirmRequest(payload) {
  ...
  try {
    ...
    if (postConfirm.status === 200 && requestType !== "PUT") {
      ...
      yield put(userSignOutSuccess());
    } else yield put(postDeliveryConfirmError("error"));
  } catch (error) {
...
function* fetchInwardInstructionDetails(action) {
  try {
    const fetchInwardInstruction = yield fetch(
      `${baseUrl}api/instruction/getById/${action.instructionId}`,
      {
        method: "GET",
        headers: getHeaders(),
      }
    ...
    if (fetchInwardInstruction.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(getInstructionByIdError("error"));
  } catch (error) {
  ...
  try {
    if (action?.inwardEntryId?.motherCoilDispatch) {
      fetchInwardInstruction = yield fetch(
        `${baseUrl}api/instruction/saveFullHandlingDispatch`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json", ...getHeaders() },
      ...
      fetchInwardInstruction = yield fetch(
        `${baseUrl}api/instruction/saveUnprocessedForDelivery/${action.inwardEntryId?.inwardEntryId}`,
        {
          method: "POST",
          headers: getHeaders(),
        }
    ...
    if (fetchInwardInstruction.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(saveUnprocessedDeliveryError("error"));
  } catch (error) {
...
function* deleteInwardEntryById(action) {
  try {
    ...
    const fetchInwardInstruction = yield fetch(
      `${baseUrl}api/inwardEntry/deleteById`,
      {
        method: "DELETE",
        body: data,
        headers: getHeaders(),
    ...
    if (fetchInwardInstruction.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(deleteInwardEntryByIdError("error"));
  } catch (error) {
...
function* deleteInstructionById(action) {
  try {
    const fetchInwardInstruction = yield fetch(
      `${baseUrl}api/instruction/${action.param}`,
      {
        method: "POST",
        headers: {
    ...
    if (fetchInwardInstruction.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(deleteInstructionByIdError("error"));
  } catch (error) {
...
function* pdfGenerateInward(action) {
  ...
  try {
    ...
    if (pdfGenerate.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(pdfGenerateError("error"));
  } catch (error) {
...
function* generateDCPdf(action) {
  try {
    ...
    if (pdfGenerate.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(generateDCPdfError("error"));
  } catch (error) {
...
function* getS3PDFUrl(action) {
  try {
    const getS3PDFUrl = yield fetch(
      `${baseUrl}api/inwardEntry/getPlanPDFs/${action.inwardEntryId}`,
      {
        method: "GET",
        headers: getHeaders(),
      }
    ...
    if (getS3PDFUrl.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(getS3PDFUrlError("error"));
  } catch (error) {
...
function* getReconcileReportSaga(action) {
  try {
    const getreconcileData = yield fetch(
      `${baseUrl}api/reports/reconcile/${action.coilNumber}`,
      {
        method: "GET",
        headers: getHeaders(),
      }
    ...
    if (getreconcileData.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(getReconcileReportError("error"));
  } catch (error) {
...
function* getPacketwisePriceDCSaga(action) {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(getPacketwisePriceDCError("error"));
  } catch (error) {
...
function* getPacketwisePriceDCFullHandlingSaga(action) {
  ...
  try {
    const response = yield call(
      fetch,
      `${baseUrl}api/delivery/validatePriceMappingFullHandling`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json", ...getHeaders() },
        body: JSON.stringify(req_obj),
    ...
    if (response.status === 200) {
      ...
      yield put(userSignOutSuccess());
    } else yield put(getPacketwisePriceDCFullHandlingError("error"));
  } catch (error) {
...
function* updateClassificationSlitAndCutBeforeFinish(action) {
  ...
  try {
    const updateClassification = yield fetch(
      `${baseUrl}api/instruction/update/classification`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json", ...getHeaders() },
        body: JSON.stringify(action.payload),
